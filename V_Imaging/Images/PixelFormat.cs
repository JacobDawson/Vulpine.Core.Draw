...
namespace Vulpine.Core.Draw.Images
{
    ...
    public struct PixelFormat
    {
        ...
        #region Vector Formats...

        public static PixelFormat Scalar
        {
            get
            {
                //defines the decoder
                DelDecodeColor dec = delegate(byte[] data, int index)
                {
                    ...
                    return Color.FromRGB(temp, temp, temp);
                };

                //defines the encoder
                DelEncodeColor enc = delegate(byte[] data, int index, Color c)
                {
                    float temp = (float)c.Luminance;
                    byte[] bits = BitConverter.GetBytes(temp);
                ...
                return new PixelFormat(32, 1, dec, enc);
            }
        }

        public static PixelFormat Vector3
        {
            get
            {
                //defines the decoder
                DelDecodeColor dec = delegate(byte[] data, int index)
                {
                    ...
                    float g = BitConverter.ToSingle(data, index + 4);
                    float b = BitConverter.ToSingle(data, index + 8);
                ...
                DelEncodeColor enc = delegate(byte[] data, int index, Color c)
                {
                    ...
                    bits = BitConverter.GetBytes((float)c.Red);
                    data[index + 0] = bits[0];
                    data[index + 1] = bits[1];
                    data[index + 2] = bits[2];
                    data[index + 3] = bits[3];

                    bits = BitConverter.GetBytes((float)c.Green);
                    data[index + 4] = bits[0];
                    data[index + 5] = bits[1];
                    data[index + 6] = bits[2];
                    data[index + 7] = bits[3];

                    bits = BitConverter.GetBytes((float)c.Blue);
                    data[index + 8] = bits[0];
                    data[index + 9] = bits[1];
                    data[index + 10] = bits[2];
                    data[index + 11] = bits[3];

                };

                return new PixelFormat(96, 3, dec, enc);
            }
        }
